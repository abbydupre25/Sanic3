...
// Code taken from here: http://slick.ninjacave.com/forum/viewtopic.php?t=1906
...
public class Camera {
	...
	private ArrayList<Integer> invisibleLayerIndex;
	...
	public Camera(GameContainer gc, TiledMap map) {
		...
		invisibleLayerIndex = new ArrayList<Integer>();
		for(int i=0; i<Defines.invisibleLayers.length; i++){
			invisibleLayerIndex.add(map.getLayerIndex(Defines.invisibleLayers[i]));
		}
	...
	public void drawMap(int offsetX, int offsetY) {
		...
		for (int i = 0; i < map.getLayerCount(); i++) {
			if (!invisibleLayerIndex.contains(i)) { // don't render invisible stuff
				map.render(tileOffsetX + offsetX, tileOffsetY + offsetY,
						tileIndexX, tileIndexY, (gc.getWidth() - tileOffsetX)
								/ tileWidth + 1, (gc.getHeight() - tileOffsetY)
								/ tileHeight + 1, i, false);
			}
		}
	}
